...
const ProductPage = ({ params }: { params: { idProduct: string } }) => {
  ...
  useEffect(() => {
    if (product?.category) {
      fetch(
        `http://localhost:3001/products/details?category=${product.category}`
      )
        .then((res) => res.json())
  ...
  return (
    <section className="py-24 flex flex-col items-center md:gap-10">
                ...
                <div className="border-b border-gray-200 py-4">
                  ...
                  {isCaractOpen && product?.caracteristics && (
                    ...
                    <div className="w-full max-w-2xl mx-auto mt-8 border border-gray-200 rounded-md overflow-hidden">
                      {Object.entries(product.caracteristics).map(
                        ([key, value], index) => (
                          <div
                            key={index}
